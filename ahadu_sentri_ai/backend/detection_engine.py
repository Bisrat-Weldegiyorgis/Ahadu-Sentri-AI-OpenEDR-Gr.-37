...
def main() -> None:
    ...
    parser.add_argument("--source-file", default=os.path.join(os.path.dirname(BASE_DIR), "data", "sample_events.json"), help="Path to sample events JSON when --source=file")

    args = parser.parse_args()

    # Allow override of env var for model
    if args.model_path:
        os.environ["SENTRI_MODEL_PATH"] = args.model_path

    loaded = load_model()  # loads model and infers required columns when possible
    ...
    for raw_event in event_iter:
        ...
        print(f"[Engine] {record['id']} label={record['predicted_label']} conf={record['confidence_score']:.2f} action={record['suggested_action']}")
